# LibrePods Windows BLE-only Fork

> **English TL;DR** — A fork of [`kavishdevar/librepods`](https://github.com/kavishdevar/librepods) attempting to port the full Linux client to Windows. Active control (noise mode switching, conversational awareness, head gestures, etc.) was abandoned because **Windows blocks userspace L2CAP socket access** at the Winsock layer (`WSAENETDOWN` even on local bind, regardless of admin elevation or BR/EDR link state). What ships here is a **BLE-only Windows tray app**: battery, in-ear detection, and proximity — everything achievable from the BLE proximity advertisement alone. For full functionality use the [original Linux/Android client](https://github.com/kavishdevar/librepods), or a USB Bluetooth dongle through [WSL2 + usbipd](./docs/wsl2-usbipd-guide.md).
>
> See [`PROBE-RESULTS.md`](./experiments/windows-feasibility/RESULTS.md) for the full evidence trail closing every standard userspace path.
...
1. **WSL2 + USB Bluetooth dongle passthrough** (`usbipd-win`). BlueZ를 WSL2에서 돌리고 외장 USB 동글을 통째로 넘겨주면 Linux 코드 그대로 동작합니다. 내장 어댑터는 passthrough 불가. 파워유저용. → [상세 가이드](./docs/wsl2-usbipd-guide.md)
